...
struct ScoringContext {
    ...
    /// Product of signed multiplier callouts aimed at `game` on `hole` (1-based); 1 when none.
    func multiplier(for game: GameID, hole: Int) -> Int {
        callouts
            .filter { $0.kind == .multiplier && $0.status == .signed && $0.params.game == game && $0.hole == hole }
