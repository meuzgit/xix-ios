-- callouts: members read; created by a member through create_callout with the caller stripped from targets
begin;
...
select is((select count(*)::int from xix.callouts where round_id = (select round from ids)), 2, 'a member reads the callouts');
select throws_ok($$insert into xix.callouts (round_id, hole, kind, caller_id, target_ids) values ((select round from fresh), 3, 'target', (select dave_row from fresh), array[(select ray_row from fresh)])$$, '42501', null, 'no direct insert');
select lives_ok($$select xix.create_callout((select round from fresh), 3::smallint, 'target', array[(select ray_row from fresh), (select dave_row from fresh)], '{"goal":"par"}'::jsonb)$$, 'a member calls out through the RPC');
select is((select target_ids from xix.callouts where round_id = (select round from fresh) and hole = 3), array[(select ray_row from fresh)], 'the caller is stripped from the targets');
select throws_ok($$select xix.create_callout((select round from fresh), 4::smallint, 'duel', array[(select ray_row from fresh), (select guest_row from fresh)], '{}'::jsonb)$$, '22023', null, 'a duel has exactly one target');
select pg_temp.as_postgres();
insert into xix.scores (round_id, player_id, hole, strokes) values ((select round from fresh), (select ray_row from fresh), 5, 4);
select pg_temp.as_user((select dave from ids));
select throws_ok($$select xix.create_callout((select round from fresh), 5::smallint, 'target', array[(select ray_row from fresh)], '{"goal":"par"}'::jsonb)$$, '55000', null, 'refused once a participant has scored the hole');
select pg_temp.as_user((select nobody from ids));
select is((select count(*)::int from xix.callouts), 0, 'a non-member sees no callouts');
select throws_ok($$select xix.create_callout((select round from fresh), 6::smallint, 'target', array[(select ray_row from fresh)], '{}'::jsonb)$$, '42501', null, 'and cannot call out');
