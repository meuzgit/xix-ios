-- confirmations: members read; one own row per round, through confirm_round
begin;
...
select lives_ok($$select xix.confirm_round((select round from ids), 'confirm')$$, 'a member confirms');
select is((select count(*)::int from xix.confirmations), 1, 'and reads confirmations');
select lives_ok($$select xix.confirm_round((select round from ids), 'object')$$, 'a second call is ignored');
select is((select action from xix.confirmations where profile_id = (select dave from ids)), 'confirm', 'the first answer stands');
select throws_ok($$insert into xix.confirmations (round_id, profile_id, action) values ((select round from ids), (select ray from ids), 'confirm')$$, '42501', null, 'cannot confirm for someone else');
select pg_temp.as_user((select nobody from ids));
select throws_ok($$select xix.confirm_round((select round from ids), 'confirm')$$, '42501', null, 'a non-member cannot confirm');
