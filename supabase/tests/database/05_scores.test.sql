...
select is((select count(*)::int from xix.scores where round_id = (select round from ids)), 72, 'a member reads every score in the round');
select lives_ok($$insert into xix.scores (round_id, player_id, hole, strokes) values ((select round from fresh), (select dave_row from fresh), 1, 4)$$, 'a member enters their own row');
select throws_ok($$insert into xix.scores (round_id, player_id, hole, strokes) values ((select round from fresh), (select ray_row from fresh), 1, 4)$$, '42501', null, 'but not another row');
select is(pg_temp.affected($$update xix.scores set strokes = 5 where round_id = (select round from fresh) and player_id = (select dave_row from fresh)$$), 1, 'a member updates their own score');
select pg_temp.as_user((select ray from ids));
select lives_ok($$insert into xix.scores (round_id, player_id, hole, strokes) values ((select round from fresh), (select guest_row from fresh), 1, 6)$$, 'the owner enters any row');
select is(pg_temp.affected($$update xix.scores set strokes = 4 where round_id = (select round from fresh) and player_id = (select dave_row from fresh)$$), 1, 'and edits any row');
select pg_temp.as_user((select nobody from ids));
select is((select count(*)::int from xix.scores), 0, 'a non-member sees no scores');
select throws_ok($$insert into xix.scores (round_id, player_id, hole, strokes) values ((select round from fresh), (select guest_row from fresh), 2, 4)$$, '42501', null, 'and cannot enter any');
