-- callout_responses: members read; a target responds once, through respond_callout, while the hole is open
begin;
...
create temp table co (id uuid);
select pg_temp.share();
select pg_temp.as_user((select ray from ids));
insert into co select (xix.create_callout((select round from fresh), 4::smallint, 'target', array[(select dave_row from fresh)], '{"goal":"par"}'::jsonb)).id;
select pg_temp.as_user((select dave from ids));
select is((select count(*)::int from xix.callout_responses), 2, 'a member reads the seeded responses');
select throws_ok($$insert into xix.callout_responses (callout_id, player_id, action) values ((select id from co), (select dave_row from fresh), 'signed')$$, '42501', null, 'no direct insert');
select lives_ok($$select xix.respond_callout((select id from co), 'signed')$$, 'a target signs through the RPC');
select is((select status from xix.callouts where id = (select id from co)), 'live', 'a signature makes the callout live');
select throws_ok($$select xix.respond_callout((select id from co), 'ducked')$$, '23505', null, 'and cannot respond twice');
select pg_temp.as_user((select ray from ids));
select throws_ok($$select xix.respond_callout((select id from co), 'signed')$$, '42501', null, 'a non-target cannot respond');
select pg_temp.as_postgres();
insert into xix.scores (round_id, player_id, hole, strokes) values ((select round from fresh), (select ray_row from fresh), 4, 4);
select is((select closed_at is not null from xix.callouts where id = (select id from co)), true, 'a participant score closes the hole');
select pg_temp.as_user((select nobody from ids));
select is((select count(*)::int from xix.callout_responses), 0, 'a non-member sees no responses');
