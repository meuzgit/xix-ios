...
**Changes from v0.2:** Pass 6 flows locked and specified (§8.16–8.22); Level displays as a number (1–10); guest accounts via anonymous auth with later linking; MMM gate copy must mirror MMM's actual allowance rule; Picked-up rule, MMM feed source and standalone XIX Full subscription confirmed.
...
- **Result-link onboarding:** guests receive the results screen by link; tapping "Confirm my scores" creates their account in one step with nothing to fill in. **Build note:** this is Supabase anonymous auth; the account must be linkable later (email or Sign in with Apple, from Settings) so a guest's rounds and medals are never stranded on one device.
