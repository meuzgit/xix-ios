...
.PHONY: engine-tests pgtap data-tests ui-tests parity wasm models reset acceptance app xcconfig app-build stickers sticker-check rankings-acceptance deploy-engine verify-engine
...
deploy-engine: wasm      ## build the module AND deploy the function together, then prove they match
	supabase functions deploy xix-engine
	./scripts/verify-deployed-engine.sh

verify-engine:           ## check the deployed engine is the one this checkout builds
	./scripts/verify-deployed-engine.sh
