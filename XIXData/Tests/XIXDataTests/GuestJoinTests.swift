...
final class GuestJoinTests: XCTestCase {
    func testGuestJoinsClaimsScoresAndOwnerCanEdit() async throws {
        ...
        let screen = try await repo.joinScreen(code: bundle.round.joinCode)
        XCTAssertEqual(screen.round.course, "Test Links")
        XCTAssertEqual(screen.round.owner, "Ray")
        XCTAssertEqual(screen.players.map(\.displayName), ["Ray", "Guest"], "every row, for the landing page")
        XCTAssertEqual(screen.claimable.map(\.displayName), ["Guest"], "only the free rows can be claimed")
        let claimed = try await repo.claimRow(roundID: round.id, playerID: screen.claimable[0].id)
